...
func TestGetSchemaNullFieldResolvedViaFallback(t *testing.T) {
	db := newTestDB(t)
	_, err := boltio.Put(db, "version", "A", []byte(`{"Edition":null}`), boltio.WriteOptions{Yes: true, Out: &bytes.Buffer{}})
	if err != nil {
		t.Fatalf("seed Put A: %v", err)
	}
	_, err = boltio.Put(db, "version", "B", []byte(`{"Edition":2}`), boltio.WriteOptions{Yes: true, Out: &bytes.Buffer{}})
	if err != nil {
		t.Fatalf("seed Put B: %v", err)
	}

	res := getSchema(t, db, "version", "A")
...
func TestGetSchemaNullFieldUnresolvedWhenNoOtherKeyResolves(t *testing.T) {
	...
	_, err = boltio.Put(db, "version", "B", []byte(`{"Edition":null}`), boltio.WriteOptions{Yes: true, Out: &bytes.Buffer{}})
	if err != nil {
		t.Fatalf("seed Put B: %v", err)
	}
...
func TestGetSchemaMultipleAmbiguousFieldsResolvedInOnePass(t *testing.T) {
	db := newTestDB(t)
	_, err := boltio.Put(db, "version", "A", []byte(`{"Edition":null,"Tags":[]}`), boltio.WriteOptions{Yes: true, Out: &bytes.Buffer{}})
	if err != nil {
		t.Fatalf("seed Put A: %v", err)
	}
	_, err = boltio.Put(db, "version", "B", []byte(`{"Edition":2,"Tags":null}`), boltio.WriteOptions{Yes: true, Out: &bytes.Buffer{}})
	if err != nil {
		t.Fatalf("seed Put B: %v", err)
	}
	_, err = boltio.Put(db, "version", "C", []byte(`{"Edition":null,"Tags":["x"]}`), boltio.WriteOptions{Yes: true, Out: &bytes.Buffer{}})
	if err != nil {
